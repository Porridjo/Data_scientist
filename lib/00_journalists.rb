...
puts nb_digits(journalists)

# plus facile ? => puts journalists.grep(/[0-9]/).count
...
puts is_aude(journalists.map(&:downcase))

# plus facile? => puts journalists.grep(/aude/).count
...
def list_under(list)
    total_under = 0
    list.each do |x|
        total_under += nb_under(x)
    end
    return total_under
end

puts list_under(journalists)

puts "Trie la liste de handle par ordre alphabétique."
journalists_sorted = journalists.sort_by {|x| x.downcase}
puts journalists_sorted

puts "Quels sont les 50 handles les plus courts de ce array ?"
journalists_short = journalists.sort_by {|x| x.length}
top50_short = journalists_short [0,49]
puts top50_short

puts "Quelle est la position dans l'array de la personne @epenser ?"

position = 0
for element in journalists
    if element == "@epenser"
        puts position
    end
    position +=1
